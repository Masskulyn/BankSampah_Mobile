...
export function AdminChatView({ onBack }: AdminChatViewProps) {
  ...
  const loadChatRooms = () => {
    try {
      ...
      setChatRooms(rooms);
      // Don't auto-select - let user click on a contact
    } catch (error) {
  ...
  // Mobile layout (< 720p): show list or chat depending on selection
  // Desktop layout (>= 720p): show list as sidebar + chat window always side by side
  if (isLargeScreen) {
    // DESKTOP: Sidebar + Chat
    return (
      <div className="h-screen flex bg-gray-50">
        {/* Sidebar - Daftar Kontak */}
        <div className="w-80 flex flex-col bg-white shadow-lg border-r border-gray-200 flex-shrink-0">
  ...
  // MOBILE: Full-screen toggle like WhatsApp
  return (
    <div className="h-screen flex bg-gray-50">
      {/* Daftar Kontak - Hidden when chat is selected */}
      {!selectedRoomId ? (
        <div className="w-full flex flex-col bg-white shadow-lg border-r border-gray-200">
          <div className="p-4 border-b border-gray-200">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-bold text-lg text-gray-900">Chat dengan User</h2>
              <Button
